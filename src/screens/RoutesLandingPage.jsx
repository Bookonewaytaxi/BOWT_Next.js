...
export default function RoutesLandingPage() {
  ...
  const fetchRoutesData = async () => {
    ...
    try {
      ...
      const cityList = Object.keys(cityMap).map(city => ({ name: city, count: cityMap[city] }));
  ...
  return (
    <>
      <Head>
        ...
        <meta property="og:url" content="https://bookonewaytaxi.in/routes" />
      ...
      <div className="min-h-screen bg-[#0F1419] text-[#E8E8E8] font-sans selection:bg-[#FFD700] selection:text-[#0F1419]">
        ...
        <div className="bg-[#161B22] border-b border-[#2F3336]">
          <div className="container mx-auto px-4 py-16 text-center">
            <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-4xl md:text-5xl font-black text-white mb-6">
              Explore Our <span className="text-[#FFD700]">Routes</span>
            </motion.h1>
            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-lg text-gray-400 max-w-2xl mx-auto mb-10">
              We connect hundreds of cities across India. Select your pickup city to see available one-way routes.
            </motion.p>
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }} className="max-w-xl mx-auto">
          ...
          {!searchTerm && (
            <section className="mb-16">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
                {popularCities.map((city) => <CityCard key={city.name} cityName={city.name} count={city.count} />)}
              </div>
            </section>
          )}
          <section>
            <div className="flex items-center gap-3 mb-8"><Search className="w-6 h-6 text-[#FFD700]" /><h2 className="text-2xl font-bold text-white">{searchTerm ? 'Search Results' : 'All Cities'}</h2></div>
            {loading ? (
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">{[...Array(8)].map((_, i) => <div key={i} className="h-32 bg-[#161B22] rounded-xl animate-pulse" />)}</div>
            ) : filteredCities.length > 0 ? (
              <>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">{paginatedCities.map((city) => <CityCard key={city.name} cityName={city.name} count={city.count} />)}</div>
                <RoutePagination currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} isLoading={false} totalItems={filteredCities.length} itemsPerPage={ITEMS_PER_PAGE} />
              </>
            ) : (
              <div className="text-center py-20 bg-[#161B22] rounded-2xl border border-[#2F3336]"><p className="text-gray-400">No cities found matching "{searchTerm}"</p></div>
            )}
          </section>
        </div>
        <Footer />
