...
export default function CityRoutesPage() {
  ...
  return (
    <>
      <Head>
        <title>Routes from {cityName} | One-Way Taxi Service</title>
        <meta name="description" content={`Explore all active one-way taxi routes from ${cityName}. Best fares guaranteed for verified cabs to popular destinations.`} />
      </Head>
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
              <div>
                <h1 className="text-3xl md:text-5xl font-black text-white mb-2">
                  Routes From <span className="text-amber-500">{cityName}</span>
                </h1>
                <p className="text-slate-400 text-lg">
                  {routes.length} active destinations available
                </p>
              </div>
              
              <div className="w-full md:w-96">
                <CitySearchBox onSearch={handleSearch} placeholder="Search destination..." />
              </div>
            </div>
          </div>
        </div>
                ...
                {paginatedRoutes.map((route) => (
                  ...
                  >
                    <span className="text-slate-800 font-medium">
                      {route.from_city || 'City'} <span className="text-slate-400">→</span> {route.to_city || 'City'}
                    </span>
