...
import { Loader2, Save, X, Sparkles, FileText, Globe } from 'lucide-react';
...
export default function CreateRouteForm() {
  ...
  const [formData, setFormData] = useState({
    ...
    seo_keywords: [],
    seo_content: '',
    seo_content_language: 'hindi', // Default Hindi
    // Validation Meta
  ...
  // Auto-generate SEO when core fields change (if SEO fields are empty)
  useEffect(() => {
  ...
  }, [formData.from_city, formData.to_city, formData.sedan_price]);
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const slug = `${slugify(formData.from_city)}-to-${slugify(formData.to_city)}-taxi`;
      ...
      toast({
        title: "Success",
        description: "Route created successfully with content",
    ...
    <div className="bg-slate-900 p-6 rounded-xl border border-slate-800 shadow-xl max-w-4xl mx-auto">
      <div className="mb-6 pb-6 border-b border-slate-800 flex justify-between items-center">
        <div>
           ...
           <p className="text-slate-400 text-sm">Fill in the information to create a new route.</p>
        </div>
      </div>
        ...
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex gap-2">
            ...
            <Button 
              ...
              disabled={!formData.from_city || !formData.to_city}
